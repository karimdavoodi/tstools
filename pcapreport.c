                        ...
                        const uint32_t len)
{
  ...
  if (!ctx->ts_r)
  {
    ...
    if (rv)
    {
      KLOG( "### pcapreport: Cannot create ts reader.\n");
      return 1;
    }
  }
                            ...
                            const uint32_t len)
{
  ...
  if (ctx->output_name)
  {
    ...
    if (rv != 1)
    {
      KLOG( "### pcapreport: Couldn't write %d bytes"
              " to %s (error = %d).\n", 
              len, ctx->output_name, 
...
int main(int argc, char **argv)
{
  ...
  while (ii < argc)
  {
    if (argv[ii][0] == '-')
    {
      ...
      else if (!strcmp("--o", argv[ii]) || !strcmp("-o", argv[ii]))
      {
        ...
        {
          KLOG( "### pcapreport: -o requires an argument.\n");
          return 1;
        }
      }
      ...
      else if (!strcmp("--d", argv[ii]) || !strcmp("-d", argv[ii]))
      {
        ...
        {
          KLOG( "### pcapreport: '%s' is not a host IP address (names are not allowed!)\n",
                  hostname);
          return 1;
        }
      }
  ...
  {
    KLOG("### pcapreport: WARNING snaplen is %d, not >= 65535 - "
...
dump_out:
      ...
      default:
        // Some other error.
        KLOG( "### pcapreport: Can't read packet %d - code %d\n",
                ctx.pkt_counter, err);
        ++done;
        break;
      }
