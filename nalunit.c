                                  ...
                                  nal_unit_context_p *context)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate NAL unit context datastructure\n");
    return 1;
  }
  new->es = es;
  new->count = 0;
...
extern int build_nal_unit(nal_unit_p  *nal)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate NAL unit datastructure\n");
    return 1;
  }
  ...
  if (err)
  {
    KLOG("### Unable to allocate NAL unit data buffer\n");
    free(new);
    return 1;
  }
                                       ...
                                       int   *rbsp_len)
{
  ...
  if (tgt == NULL)
  {
    KLOG("### Cannot malloc RBSP target array\n");
    return 1;
  }
...
static inline int prepare_rbsp(nal_unit_p  nal)
{
  ...
  if (nal->rbsp == NULL)
  {
    ...
    if (err)
    {
      KLOG("### Error removing emulation prevention bytes\n");
      return 1;
    }
  }
  ...
  if (err)
  {
    KLOG("### Unable to build bitdata datastructure for NAL RBSP\n");
    return 1;
  }
  nal->bit_data = bd;
  return 0;
...
#define CHECK(name) \
  if (err)                                      \
  {                                             \
    KLOG("### Error reading %s field from slice data\n",(name)); \
  }
...
#define CHECK(name)                                                     \
  if (err)                                                              \
  {                                                                     \
    KLOG("### Error reading %s field from picture parameter set\n", \
            (name));                                                    \
  }
...
#define CHECK(name)                                                     \
  if (err)                                                              \
  {                                                                     \
    KLOG("### Error reading %s field from sequence parameter set\n",\
            (name));                                                    \
  }
  ...
  {
    KLOG("### reserved_zero_5bits not zero (%d) in sequence"
...
#define CHECK(name)                                         \
  if (err)                                                  \
  {                                                         \
    KLOG("### Error reading %s field from SEI\n", \
            (name));                                        \
  }
                          ...
                          int          show_nal_details)
{
  ...
  if (err)
  {
    KLOG("### Error reading RBSP data for %s NAL (ref idc %x,"
                                ...
                                nal_unit_p   last)
{
  ...
  if (!nal->decoded)
  {
    KLOG("### Cannot decide if NAL unit is first VCL NAL\n"
                          ...
                          int         show_nal_details)
{
  ...
  if (nal == NULL)
  {
    KLOG("### Attempt to check profile on a NULL NAL unit\n");
    return;
  }
  else if (nal->nal_unit_type != 7)
  {
    KLOG("### Attempt to check profile on a NAL unit that is not a "
  ...
  else if (!nal->decoded)
  {
    ...
    if (err)
    {
      KLOG("### Error trying to decode RBSP for first sequence"
              " parameter set\n");
      return;
    }
  }
  ...
  else
  {
    int sum = data.constraint_set0_flag + data.constraint_set1_flag +
      data.constraint_set2_flag;
    KLOG("\n");
    KLOG("Warning: This bitstream declares itself as %s profile (%d)",
    ...
    {
      KLOG(",\n");
      KLOG("         and as obeying the constraints of the");
      if (data.constraint_set0_flag) KLOG(" baseline");
      if (data.constraint_set1_flag) KLOG(" main");
      if (data.constraint_set2_flag) KLOG(" extended");
      KLOG(" profile%s.\n",(sum==1?"":"s"));
    }
    KLOG("         This software does not support %s profile,\n",
            name);
    KLOG("         and may give incorrect results or fail.\n\n");
    return;
  }
}

                          ...
                          nal_unit_p  nal)
{
  ...
  {
    KLOG("### NAL forbidden_zero_bit is non-zero, at "
            OFFSET_T_FORMAT "/%d\n",
            nal->unit.start_posn.infile,nal->unit.start_posn.inpacket);
    KLOG("    First byte of NAL unit is %02x",nal->data[0]);
    if (nal->data[0] == 0xB3)
      KLOG(", which is H.262 sequence header start code\n"
                              ...
                              nal_unit_p         *nal)
{
  ...
  if (nal_is_pic_param_set(*nal))
  {
    ...
    if (err)
    {
      KLOG("### Error remembering picture parameter set ");
      report_nal(stderr,*nal);
      free_nal_unit(nal);
      return 1;
    }
  ...
  else if (nal_is_seq_param_set(*nal))
  {
    ...
    if (err)
    {
      KLOG("### Error remembering sequence parameter set ");
      report_nal(stderr,*nal);
      free_nal_unit(nal);
      return 1;
    }
                                ...
                                nal_unit_p  nal)
{
  ...
  if (err)
  {
    KLOG("### Error writing NAL unit as ES\n");
    return err;
  }
  else
    return 0;
                                ...
                                uint32_t    video_pid)
{
  ...
  if (err)
  {
    KLOG("### Error writing NAL unit as TS\n");
    return err;
  }
  else
    return 0;
...
extern int build_param_dict(param_dict_p *param_dict)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate parameter 'dictionary'"
            " datastructure\n");
    return 1;
  }
  ...
  if (new->ids == NULL)
  {
    KLOG("### Unable to allocate array within 'dictionary'"
            " datastructure\n");
    free(new);
    return 1;
  }
                               ...
                               nal_unit_p    nal)
{
  ...
  if (param_dict->length == param_dict->size)
  {
    ...
    {
      KLOG("### Unable to extend parameter set dictionary array\n");
      return 1;
    }
                              ...
                              nal_pic_param_data_p *pic_param_data)
{
  ...
  if (absent)
  {
    KLOG("### Unable to find picture parameter set with id %u\n",
            pic_param_id);
    return 1;
  }
                              ...
                              nal_seq_param_data_p *seq_param_data)
{
  ...
  if (absent)
  {
    KLOG("### Unable to find sequence parameter set with id %u\n",
            seq_param_id);
    return 1;
  }
...
extern int build_nal_unit_list(nal_unit_list_p  *list)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate NAL unit list datastructure\n");
    return 1;
  }
                                   ...
                                   nal_unit_p       nal)
{
  if (list->length == list->size)
  {
    ...
    if (list->array == NULL)
    {
      KLOG("### Unable to extend NAL unit list array\n");
      return 1;
    }
    list->size = newsize;
  }
