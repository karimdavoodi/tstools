    ...
    for (;;)
    {
      ...
      else if (stream_id == PRIVATE1_AUDIO_STREAM_ID)
      {
        ...
        if (what != SUBSTREAM_ERROR)
        {
          ...
          {
            KLOG("Internal error: got substream index %d"
          ...
          default: KLOG("Internal error: got substream id %d"
...
int main(int argc, char **argv)
{
  ...
  if (!had_input_name)
  {
    KLOG("### psreport: No input file specified\n");
    return 1;
  }
  ...
  if (err)
  {
    KLOG("### psreport: Unable to open input file %s\n",
  ...
  if (err)
    KLOG("### psreport: Error reporting on input stream\n");

  err = close_PS_file(&ps);
  if (err)
  {
    KLOG("### psreport: Error closing input file %s\n",
            (use_stdin?"<stdin>":input_name));
    return 1;
  }
  return 0;
