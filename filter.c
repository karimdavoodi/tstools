...
static int new_h262_filter_context(h262_filter_context_p *fcontext)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate H.262 filter context\n");
    return 1;
  }
  new->h262 = NULL;
...
static int new_h264_filter_context(h264_filter_context_p *fcontext)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate H.264 filter context\n");
    return 1;
  }
                                        ...
                                        int                   *frames_seen)
{
  ...
  *frames_seen = 0;
  ...
  if (fcontext->filter)
  {
    KLOG("### Calling get_next_stripped_h262_frame with a context"
            " set for filtering\n");
    return 1;
  }
  ...
  for (;;)
  {
    ...
    {
      KLOG("### Error filtering H.262 frames\n");
      return 1;
    }
                                        ...
                                        int                   *frames_seen)
{
  ...
  *frames_seen = 0;
  ...
  if (!fcontext->filter)
  {
    KLOG("### Calling get_next_filtered_h262_frame with a context"
            " set for stripping\n");
    return 1;
  }
