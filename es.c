                                  ...
                                  ES_p  *es)
{
  ...
  if (err)
  {
    KLOG("### Error building elementary stream for file %s\n",
            filename);
    return 1;
  }
  return 0;
...
static int setup_readahead(ES_p  es)
{
  ...
  if (es->reading_ES)
  {
    ...
    {
      KLOG("### File only contains %d byte%s\n",
  ...
  else
  {
    ...
    {
      KLOG("### File PES packet only contains %d byte%s\n",
                                        ...
                                        ES_p  *es)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate elementary stream datastructure\n");
    return 1;
  }
...
extern int setup_ES_unit(ES_unit_p  unit)
{
  ...
  if (unit->data == NULL)
  {
    KLOG("### Unable to allocate ES unit data buffer\n");
    return 1;
  }
  unit->data_len = 0;
...
extern int build_ES_unit(ES_unit_p  *unit)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate ES unit datastructure\n");
    return 1;
  }
  ...
  for (;;)
  {
    ...
    for (ptr = es->data_ptr; ptr < es->data_end; ptr++)
    {
      ...
      if (unit->data_len == unit->data_size)
      {
        ...
        if (unit->data == NULL)
        {
          KLOG("### Unable to extend ES unit data array\n");
          return 1;
        }
                         ...
                         ES_unit_p  unit)
{
  ...
  {
    KLOG("### Error writing out ES unit data: %s\n"
                       ...
                       ES_offset  where)
{
  ...
  if (es->reader == NULL)
  {
    KLOG("### Attempt to seek in PES for an ES reader that"
  ...
  if (err)
  {
    KLOG("### Error seeking for PES packet at " OFFSET_T_FORMAT
            "\n",where.infile);
    return 1;
  }
  ...
  if (err)
  {
    KLOG("### Error reading PES packet at " OFFSET_T_FORMAT "/%d\n",
  ...
  {
    KLOG("### Error seeking PES packet at " OFFSET_T_FORMAT "/%d: "
                   ...
                   ES_offset  where)
{
  ...
  if (es->reading_ES)
  {
    ...
    if (err)
    {
      KLOG("### Error seeking within ES file\n");
      return 1;
    }
  }
  else
                        ...
                        byte     **data)
{
  ...
  if (*data == NULL || data_len == NULL || num_bytes > *data_len)
  {
    *data = realloc(*data,num_bytes);
    if (*data == NULL)
    {
      KLOG("### Unable to reallocate data space\n");
      return 1;
    }
    if (data_len != NULL)
      *data_len = num_bytes;
  ...
  if (es->reading_ES)
  {
    ...
    if (err)
    {
      if (err == EOF)
      {
        KLOG("### Error (EOF) reading %d bytes\n",num_bytes);
        return 1;
      }
      else
        return err;
  ...
  else
  {
    ...
    if (err)
    {
      KLOG("### Error reading %d bytes from PES\n",num_bytes);
      return 1;
    }
  }
                                            ...
                                            int        *data_len)
{
  ...
  if (es->reading_ES)
  {
    KLOG("### Cannot retrieve end of PES packet - the ES data"
  ...
  *data = malloc(*data_len);
  if (*data == NULL)
  {
    KLOG("### Cannot allocate space for rest of PES packet\n");
    return 1;
  }
...
extern int build_ES_unit_list(ES_unit_list_p  *list)
{
  ...
  if (new == NULL)
  {
    KLOG("### Unable to allocate ES unit list datastructure\n");
    return 1;
  }
                                  ...
                                  ES_unit_p       unit)
{
  ...
  if (list->length == list->size)
  {
    ...
    if (list->array == NULL)
    {
      KLOG("### Unable to extend ES unit list array\n");
      return 1;
    }
    list->size = newsize;
  }
  ...
  *ptr = *unit;
  ...
  if (ptr->data == NULL)
  {
    KLOG("### Unable to copy ES unit data array\n");
    return 1;
  }
                                ...
                                int  *video_type)
{
  ...
  *video_type = VIDEO_UNKNOWN;
  ...
  if (err)
  {
    KLOG("### Error trying to setup ES unit before"
                                     ...
                                     int  *video_type)
{
  ...
  if (start_posn == -1)
  {
    KLOG("### Error remembering start position in file before"
            " working out video type\n");
    return 1;
  }
  ...
  if (err)
  {
    KLOG("### Error rewinding file before"
            " working out video type\n");
    return 1;
  }
  ...
  if (err)
  {
    KLOG("### Error deciding video type of file\n");
    free_elementary_stream(&es);
    return 1;
  }
  ...
  if (err)
  {
    KLOG("### Error returning to start position in file after"
            " working out video type\n");
    return 1;
  }
  return 0;
